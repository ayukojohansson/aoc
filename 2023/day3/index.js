...
const realInput = `.......262....300...................507.....961..............668.....................189.906...........................624..................
...
const getNumbers = (line) => {
  ...
  return numbers.map(n => {
    const index = line.indexOf(n, previousIndex);
    previousIndex = index + n.length;
    return { number: n, x: index, isValid: false };
  })
}

const getSymbols = (line, y) => {
  const symbols = line.match(/([^\d.])/g);
  if (!symbols) {
    return [];
  }
  let previousIndex = 0;
  return symbols.map((s, i) => {
    const index = line.indexOf(s, previousIndex);
    previousIndex = index + s.length;
    return {symbols: s, x: index, y};
  })
}
...
const part1 = (input) => {
  ...
  const symbols = lines.flatMap(getSymbols);
  symbols.forEach(s => {
    numbers[s.y-1].forEach(n => {
      if (n.x-1 <= s.x && n.x+n.number.length >= s.x) {
        // console.log(n, 'UP now valid by', s)
        n.isValid = true;
      }
    })
    numbers[s.y].forEach(n => {
      if (n.x-1 == s.x || n.x+n.number.length == s.x) {
        // console.log(n, 'MID now valid by', s)
        n.isValid = true;
      }
    })
    numbers[s.y+1].forEach(n => {
      if (n.x-1 <= s.x && n.x+n.number.length >= s.x) {
        // console.log(n, 'BOT now valid by', s)
        n.isValid = true;
      }
    })
  });
  ...
  // console.log(symbols)
  // console.log(numbers)
  const sum = numbers.flatMap(x => x).reduce((sum, curr) => {
    // console.log(numbers)
    return curr.isValid ? sum + parseInt(curr.number, 10) : sum
  }, 0)
  console.log(sum)
}
...
const part2 = (input) => {
  ...
  symbols.forEach(s => {
    ...
    numbers[s.y+1].forEach(n => {
      if (n.x-1 <= s.x && n.x+n.number.length >= s.x) {
        // console.log(n, 'LOW now valid by', s)
        gear.push(n);

      }
    })
    if (gear.length == 2) {
      sum += parseInt(gear[0].number, 10) * parseInt(gear[1].number, 10)
    }
  });
