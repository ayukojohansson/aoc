...
const realInput = `Game 1: 2 green, 12 blue; 6 red, 6 blue; 8 blue, 5 green, 5 red; 5 green, 13 blue; 3 green, 7 red, 10 blue; 13 blue, 8 red
...
const getStats = (line) => {
  const stats = line.split(`:`)[1].split(';').map(game => {
    // game = 1 red, 2 green, 6 blue
    const red = game.match(/(\d+)\sred/);
    const blue = game.match(/(\d+)\sblue/);
    const green = game.match(/(\d+)\sgreen/);
    return {
      red: parseInt(red?.[1] || 0, 10),
      blue: parseInt(blue?.[1] || 0, 10),
      green: parseInt(green?.[1] || 0, 10),
    }
  })
  return stats;
}
...
const part1 = (input) => {
  ...
  const games = lines.map(getStats);

  // 12 red cubes, 13 green cubes, and 14 blue cubes
  const validGames = games.reduce((sum, g, i) => {
    return g.every(isValid) ? sum + i+1 : sum;
  }, 0)
...
const getMinCubes = (stats) => stats.reduce(({ red, blue, green}, curr) => {
  return {
    red: red > curr.red ? red : curr.red,
    blue: blue > curr.blue ? blue : curr.blue,
    green: green > curr.green ? green : curr.green,
  }
})
...
const part2 = (input) => {
  ...
  const sum = games.reduce((sum, g) => {
    const { red, green, blue} = getMinCubes(g);
