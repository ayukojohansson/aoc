...
//  let x, y;
  ...
  const checkHash = (r, lastKey, step) => {
    const hash = [...new Set(r.replace(/[A-Z]/g, '').split(''))].sort().join('') + lastKey;
...
//    console.log('investigating', route);

    const currentBots = route.slice(-4);
    let possibleCounter = 4;
    for(var i=0;i<4;i++) {
      const [x, y] = mapR[currentBots.slice(i,i+1)];
      possibilities = findNearest(currentMap, x, y, route);
//      console.log('possibilities', possibilities)
      possibilities.forEach(p => {
  //      console.log(p)
        const r = route + currentBots.replace(currentBots[i], p.status);
        const s = step + p.step;
        if (p.isKey && checkHash(route, p.status, s) ) {
          if (step < min) {
            possibleRoute.push({
              route: r,
              step: s,
