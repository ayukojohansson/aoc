...
const run = (INPUT, intruction, name) => {
  ...
  const setCanvas = (x,y,s) => {
    ...
    canvas[`${x},${y}`].back = returnMapping[returnWay];
  }
...
//          ascii(canvas);
    ...
    const currentStatus = getStatus(x,y);
    while (command) {
      temp = getPos(command);
      const status = getStatus(...temp)
      if (!status || status.step > currentStatus.step + 1)
        break;
...
//    console.log('input is', command);
  ...
  let currentStep = 0;
  let currentLoop = 0;
  while (!shouldHalt && currentLoop < 100000) {
