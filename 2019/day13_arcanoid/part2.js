...
const run = (INPUT, intruction, name) => {
  ...
  let score;
  let canvas = Array(WIDTH * HEIGHT).fill(0);
  let ball, joy;
  const findInput = () => {
    while (OUTPUT.length >= 3) {
      const [x, y, paint] = OUTPUT.splice(0,3);
      if (x== -1 && y == 0) {
        score = paint;
      } else {
        canvas[x + y * WIDTH] = paint;
        if (paint == 3) {
          joy = x;
        }
        if (paint == 4) {
    ...
    if (ball < joy) INPUT.push(-1)
    if (ball == joy) INPUT.push(0)
    if (ball > joy) INPUT.push(1)
  ...
  findInput();
  logger.result('Score', score)
  paintArray(canvas, WIDTH);
  return [OUTPUT, intruction];
};

const paintArray = (arr, width) => {
    while (arr.length) {
    logger.result(
      arr.splice(0, width)
        .map(d => d==0 ? ' ' : d==1 ? '|' : d==2 ? '.' : d)
